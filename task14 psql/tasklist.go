...
func (c *TaskList) UpdateTask(conn *pgxpool.Pool, taskID int) pgx.Rows {
	_, err := conn.Exec(context.Background(), "update tasklist set task=$2, updated_at=$3, contact_id=$4 where id=$1",
...
func (c *TaskList) DeleteTask(conn *pgxpool.Pool, taskID int) {
	_, err := conn.Exec(context.Background(), "delete from tasklist where id=$1", taskID)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Query update failed: %v\n", err)
	}else {
